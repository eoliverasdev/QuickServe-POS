...
<head>
    ...
    <style>
        ...
        .historial-date-field input[type="date"] {
            padding: 9px 12px;
            border: 1px solid #e2e8f0;
            border-radius: 10px;
            font-size: 0.9rem;
        ...
        .historial-date-field input[type="date"]:focus {
            outline: none;
            border-color: #4e73df;
...
<body>
                ...
                <div class="historial-filters">
                    <div class="historial-filters-row">
                        ...
                        <form class="historial-date-form" action="{{ url('/admin') }}" method="GET" id="historial-date-filter-form">
                            ...
                            <div class="historial-date-row">
                                <label class="historial-date-field">
                                    <span>Des de</span>
                            ...
                            <div class="historial-date-actions">
                                <button type="submit" class="btn btn-add">Aplicar període</button>
                                @if($startDate || $endDate)
                                    <a href="?payment={{ $paymentFilter ?? 'all' }}#comandes" class="btn btn-delete" style="text-decoration:none; display:inline-flex; align-items:center; justify-content:center;" title="Netejar dates">Netejar dates</a>
                                @endif
                            </div>
                        </form>
                    </div>
                    ...
                    @if($startDate || $endDate)
                        <div class="historial-range-info">
                            Mostrant vendes
                            @if($startDate)
                                des de <strong>{{ \Carbon\Carbon::parse($startDate)->format('d/m/Y') }}</strong>
                            @endif
                            @if($endDate)
                                fins a <strong>{{ \Carbon\Carbon::parse($endDate)->format('d/m/Y') }}</strong>
                            @endif
                            @if($paymentFilter && $paymentFilter !== 'all')
                                · mètode <strong>{{ $paymentFilter }}</strong>
                            @endif
                        </div>
                    @endif
                </div>
    ...
    <script>
        ...
        // ── Historial: el formulari de dates manté l'ancla #comandes ──
        document.addEventListener('DOMContentLoaded', () => {
            const form = document.getElementById('historial-date-filter-form');
            if (!form) return;
            form.addEventListener('submit', (e) => {
                ...
                window.location.href = base + (qs ? ('?' + qs) : '') + '#comandes';
            });
        });
